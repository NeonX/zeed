...
$(function () {
        ...
        beforeLoad: function () {
            ...
            if (mode == 'new') {
                ...
                $.ajax({
                    ...
                    datatype    : 'json',
                    url         : self.getByIdURL,
                    data        : { table: self.table, customertype_id: $(_self.element.innerHTML).get(0).getAttribute('value') },
    ...
    self.custom.bind('submit', function () {
     ...
     self.del.bind('click', function () {
        $.ajax({
            ...
            cache       : false,
            url         : self.saveURL,
            data        : {table: self.table, customertype_id: $('#' + self.colId).val(), mode: 'delete', deleteflag: 1 },
            success     : function (data) {
                $.fancybox(data);
                self.grid.trigger('reloadGrid');
            }
    ...
    self.recovery.bind('click', function () {
        $.ajax({
            ...
            cache       : false,
            url         : self.saveURL,
            data        : {table: self.table, customertype_id: $('#' + self.colId).val(), mode: 'delete', deleteflag: 0 },
            success     : function (data) {
                $.fancybox(data);
                self.grid.trigger('reloadGrid');
            }
